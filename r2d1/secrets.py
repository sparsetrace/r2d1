...
Search order (does not override existing env vars):
  1. .env in current directory or parents
  2. os.environ  (Modal, Vast.ai, RunPod, Docker, CI, SageMaker, Vertex, etc.)
...
# ── Alias table ───────────────────────────────────────────────────────────────
# R2D1_* names take priority; generic / AWS names are fallbacks.
DEFAULT_SECRET_ALIASES: dict[str, tuple[str, ...]] = {
    "HF_TOKEN":              ("HF_TOKEN", "HF_HUB_TOKEN", "HUGGINGFACE_TOKEN", "HUGGINGFACE_HUB_TOKEN"),
    ...
    # Cloudflare / R2D1
    "R2D1_ACCOUNT_ID":       ("R2D1_ACCOUNT_ID", "CLOUDFLARE_ACCOUNT_ID", "CF_ACCOUNT_ID"),
    "R2D1_API_TOKEN":        ("R2D1_API_TOKEN", "CLOUDFLARE_API_TOKEN", "CF_API_TOKEN"),
    "R2D1_D1_DATABASE_ID":   ("R2D1_D1_DATABASE_ID", "D1_DATABASE_ID", "CLOUDFLARE_D1_DATABASE_ID", "CF_D1_DATABASE_ID"),
    "R2D1_R2_BUCKET":        ("R2D1_R2_BUCKET", "R2_BUCKET", "CLOUDFLARE_R2_BUCKET", "CF_R2_BUCKET"),
    "R2D1_R2_ACCESS_KEY":    ("R2D1_R2_ACCESS_KEY", "R2_ACCESS_KEY", "R2_ACCESS_KEY_ID", "CLOUDFLARE_R2_ACCESS_KEY", "AWS_ACCESS_KEY_ID"),
    "R2D1_R2_SECRET_KEY":    ("R2D1_R2_SECRET_KEY", "R2_SECRET_KEY", "R2_SECRET_ACCESS_KEY", "CLOUDFLARE_R2_SECRET_KEY", "AWS_SECRET_ACCESS_KEY"),
    "R2D1_R2_ENDPOINT_URL":  ("R2D1_R2_ENDPOINT_URL", "R2_ENDPOINT_URL", "CLOUDFLARE_R2_ENDPOINT_URL", "AWS_ENDPOINT_URL_S3"),
}

COMMON_OPTIONAL_SECRETS: tuple[str, ...] = (
    "HF_TOKEN", "GITHUB_TOKEN", "GITLAB_TOKEN", "WANDB_API_KEY",
    "KAGGLE_USERNAME", "KAGGLE_KEY", "OPENAI_API_KEY", "ANTHROPIC_API_KEY",
    "GOOGLE_API_KEY", "COMET_API_KEY", "NEPTUNE_API_TOKEN",
    "MODAL_TOKEN_ID", "MODAL_TOKEN_SECRET", "VAST_API_KEY",
)

R2_REQUIRED: tuple[str, ...] = (
    "R2D1_ACCOUNT_ID", "R2D1_R2_BUCKET", "R2D1_R2_ACCESS_KEY", "R2D1_R2_SECRET_KEY",
)

D1_OPTIONAL: tuple[str, ...] = (
    "R2D1_API_TOKEN", "R2D1_D1_DATABASE_ID",
)
...
def _locations_text() -> str:
    return (
        "local .env files, os.environ, Google Colab userdata, Kaggle UserSecretsClient.\n"
        "Platform-injected secrets (Modal, Vast.ai, RunPod, Docker, CI, SageMaker, "
        "Vertex, Lightning AI) are covered by os.environ."
    )
...
def lookup_secret(
    ...
    *,
    ...
    return SecretLookup(name=name, value=value, found_as=found_as,
                        source=source, tried_names=tried)
...
def secret(
    ...
    *,
    ...
    """Resolve a secret across all supported environments."""
    info = lookup_secret(name, aliases=aliases, set_env=set_env,
                         env_name=env_name, dotenv_path=dotenv_path,
...
def export_secrets(
    ...
    *,
    ...
    """Resolve several secrets and populate os.environ for any found."""
...
def discover_common_secrets(*, required: bool = False) -> dict[str, Optional[str]]:
    """Opportunistically load common ML/dev tokens."""
    return export_secrets(COMMON_OPTIONAL_SECRETS, required=required)


def r2d1_config(*, discover_common: bool = True) -> dict[str, Optional[str]]:
    """Resolve the full r2d1 credential bundle without raising."""
    if discover_common:
